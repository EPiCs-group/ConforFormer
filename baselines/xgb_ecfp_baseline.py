#!/usr/bin/env python3
"""Train XGBoost baselines on Uni-Mol property benchmarks using RDKit ECFP fingerprints."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("results/xgb_ecfp4_1024"),
        help="Where summary CSV/JSON outputs are written",
    )
    return parser.parse_args()


def fingerprint_name(radius: int, n_bits: int) -> str:
    return f"ECFP{radius * 2}_{n_bits}"
...
def ecfp_on_bits(
    ...
    return np.fromiter(fp.GetOnBits(), dtype=np.int32)
...
def build_fp_cache(
    ...
    onbits_map: dict[str, np.ndarray] = {}
    failures = 0
    ...
    for smi in uniq:
        bits = ecfp_on_bits(smi, generator)
        if bits is None:
            failures += 1
            continue
        onbits_map[smi] = bits

    return onbits_map, failures
...
def build_feature_matrix(
    ...
    for i, smi in enumerate(smiles):
        bits = onbits_map.get(smi)
        if bits is None:
            indptr.append(len(indices))
            continue

        keep[i] = True
        indices.extend(bits.tolist())
        data.extend([1.0] * int(bits.size))
        indptr.append(len(indices))
