...
class UniMolModel(BaseUnicoreModel):
    ...
    def register_large_classification_head(
        self, name, num_classes=None, inner_dim=None, **kwargs
    ):
        """Register a classification head."""
        if name in self.classification_heads:
        ...
        self.classification_heads[name] = LargeClassificationHead(
            input_dim=self.args.encoder_embed_dim,
            inner_dim_1=256,
            inner_dim_2=128,
            num_classes=num_classes,
...
class LargeClassificationHead(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.dense1 = nn.Linear(input_dim, inner_dim_1)
        self.dense2 = nn.Linear(inner_dim_1, inner_dim_2)
        self.out_proj = nn.Linear(inner_dim_2, num_classes)
    ...
    def forward(self, features, **kwargs):
        ...
        x = self.dense1(x)   #512 -> inner dim
        x = self.activation_fn(x)
        x = self.dropout(x)
        x = self.dense2(x)   #512 -> inner dim
        x = self.activation_fn(x)
        x = self.dropout(x)
        x = self.out_proj(x)
        return x
