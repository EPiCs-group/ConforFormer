...
class UniMolModel(BaseUnicoreModel):
    ...
    def __init__(self, args, dictionary):
        ...
        channels = 64
        self.embed_gbf = GaussianLayer(channels, n_edge_type)
        self.embedding_proj = NonLinearHead( # for the projecting of the gassian functions to the embedding dimension
            channels, args.encoder_embed_dim, args.activation_fn
        )
    ...
    def forward(
        ...
        **kwargs
        ...
        def get_atom_embedding(dist, et, padding_mask):
            gbf_feature = self.embed_gbf(dist, et)
            mask_expanded = padding_mask.unsqueeze(1).to(bool)
            attn_scores = gbf_feature.mean(dim=-1)
            attn_scores = attn_scores.masked_fill(mask_expanded, float('-inf'))
            attn_weights = F.softmax(attn_scores, dim=-1)
            context = torch.einsum('bij,bijk->bik', attn_weights, gbf_feature)
            return self.embedding_proj(context)
        
        mask = (1-padding_mask.to(dtype=graph_attn_bias.dtype))
        x = get_atom_embedding(src_distance, src_edge_type, padding_mask)
