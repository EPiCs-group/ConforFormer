...
@register_model("contrast")
class UniMolModel_contrast(BaseUnicoreModel):
    @staticmethod
    def add_args(parser):
        ...
        parser.add_argument(
            ...
            type=float,
            default=0.1,
            help="temperature hyperparamter for the contrastive loss"
        )
        parser.add_argument(
            "--mode",
            type=str,
    ...
    def __init__(self, args, dictionary):
        # initialize archetecture and parser args
        super().__init__()
    ...
    def forward(
        ...
        **kwargs
        ...
        ### these can be removed once we start training
        # assert src_distance.size(0) == src_coord.size(0)
        # assert src_tokens.size(0) == src_edge_type.size(0)
        # assert src_coord.size(0) % src_tokens.size(0) == 0
        # assert src_coord.size(0) % src_edge_type.size(0) == 0
        ###

        times_to_repeat = int(src_coord.size(0) / src_tokens.size(0))
        
        src_tokens = src_tokens.repeat_interleave(times_to_repeat, dim=0)
        src_edge_type = src_edge_type.repeat_interleave(times_to_repeat, dim=0)

        # print(f"{src_tokens.shape=}")
        # print(f"{src_distance.shape=}")
        # print(f"{src_coord.shape=}")
        # print(f"{src_edge_type.shape=}")

        if classification_head_name is not None:
            features_only = True

        # padding to remove tokens required for tensor creation
        padding_mask = src_tokens.eq(self.padding_idx)
        if not padding_mask.any():
            # padding_mask = None
            padding_mask = torch.zeros_like(src_tokens)
        # Atom embedding using "static" embedding depening solely on atom type
        x = self.embed_tokens(src_tokens)

        def get_dist_features(dist, et):
            """
            Use gaussian basis functions to determine the graph attention bias.
            Args:
              dist: Distances between each respective atoms
              et: 'tokenized' edge type. Defines between which two atoms the edge is
            """
            n_node = dist.size(-1)
        ...
        if not features_only:
            if self.args.masked_token_loss > 0:
                # determine logits - basically asking the model to predict which atom is being masked/hidden
                logits = self.lm_head(encoder_rep, encoder_masked_tokens)

            if self.args.masked_coord_loss > 0:
                coords_emb = src_coord
                # create the padding mask to hide the coordinates which are not actually part of the atom
                ...
                #
                coord_update = torch.sum(coord_update, dim=2) # updating coordinates for the de-noising process
                encoder_coord = coords_emb + coord_update     # final predictions for the coordinates which will be assessed
        ...
        if self.args.mode == 'infer':
            return encoder_rep, encoder_pair_rep, logits
        else:
            cls_token = encoder_rep[:, 0, :]
            return (
                logits,
                encoder_distance,
                encoder_coord,
...
@register_model_architecture("contrast", "contrast")
...
@register_model_architecture("contrast", "unimol_contrast")
def unimol_contrast_architecture(args):
    base_architecture(args)
