...
# This class is not currently in use because it is dumb. Why would be add non-physical 2D molecules to the dataset
class Add2DConformerDataset(BaseWrapperDataset):
    ...
    def __cached_item__(self, index: int, epoch: int):
        ...
        coordinates_2d = smi2_2Dcoords(smi, len(atoms))
...
def smi2_2Dcoords(smi, num_atoms):
    mol = Chem.MolFromSmiles(smi)
    if mol is None:
        default_smi_string = "".join(["C" for _ in range(num_atoms)])
        mol = Chem.MolFromSmiles(default_smi_string)
