...
class UniMolFinetuneTask(UnicoreTask):
    ...
    def add_args(parser):
        ...
        parser.add_argument(
            "--freeze-layers",
            action="store_true",
            help="Freeze all of the model weights except the classification head",
        )
        parser.add_argument(
            "--unfreeze-last-layer",
            action="store_true",
            help="Unfreeze the final layer of the model",
        )
        parser.add_argument(
        ...
        parser.add_argument(
            "--use-large-MLP",
            action="store_true",
            help="Use a 3 layer MLP instead of a 2 layer",
        )
    ...
    def build_model(self, args):
        ...
        if args.freeze_layers:
            ...
            # unfreeze the last layer (if we want)
            if args.unfreeze_last_layer:
                for param in model.encoder.layers[-1].parameters():
                    param.requires_grad = True
            
            if args.unfreeze_last_3_layers:
                for layer_num in [-1, -2, -3]:
                    for param in model.encoder.layers[layer_num].parameters():
                        param.requires_grad = True
            
            for name, param in model.named_parameters():
                print(name, param.requires_grad)
        
        if args.use_large_MLP:
            model.register_large_classification_head(
