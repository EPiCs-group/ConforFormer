...
@register_task("unimol_contrast_head")
class ContrastHeadTask(UnicoreTask):
    ...
    def load_dataset(self, split, combine=False, **kwargs):
        ...
        def one_dataset(raw_dataset, coord_seed, mask_seed):
            ...
            contrast_dataset = RemoveHydrogenDataset(
                dataset,
                "atoms",
                "coordinates_2",
            ...
            contrast_dataset = CroppingDataset(
                contrast_dataset, self.seed, "atoms", "coordinates_2", self.args.max_atoms
            ) # Safety for incase there are too many atoms within the molecule. Stictly speaking this in unecessary
            contrast_dataset = NormalizeDataset(contrast_dataset, "coordinates_2", normalize_coord=True) # "normalizing" in this case means centering the atom
            ...
            dataset = CroppingDataset(
                dataset, self.seed, "atoms", "coordinates_1", self.args.max_atoms
            ) # Safety for incase there are too many atoms within the molecule. Stictly speaking this in unecessary
            dataset = NormalizeDataset(dataset, "coordinates_1", normalize_coord=True) # "normalizing" in this case means centering the atom
            ...
            coord_dataset = KeyDataset(dataset, "coordinates_1") # extract the coordinates of the chosen atom
            ...
            contrast_coords = KeyDataset(contrast_dataset, "coordinates_2")
        ...
        net_input, target, contrast = one_dataset(raw_dataset, self.args.seed, self.args.seed)
        dataset = {"net_input": net_input, "target": target, "contrast_input": contrast}
