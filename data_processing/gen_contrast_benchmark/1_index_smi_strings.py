...
def process_file(worker_id, input_queue: Queue, writer_queue: Queue, summary_queue: Queue):
        ...
        with open(smi_file, "r") as f:
            ...
            for i, row in enumerate(reader):
                ...
                if chem_formula is not None:
                    writer_queue.put((chem_formula, ZINC_index))  # put into the lmdb
                    summary_queue.put((chem_formula))             # to keep track of occurance
...
def lmdb_writer(writer_id, writer_queue, lmdb_path):
    ...
    map_size = 100 if writer_id == 0 else 50
    ...
    while True:
        ...
        formula, ZINC_id = new_input
        
        buffer[formula].add(ZINC_id)
        elem_in_buffer += 1
        if elem_in_buffer >= 250_000:
            batch_write(buffer)
            buffer.clear()
            elem_in_buffer = 0
...
def summary_writer(summary_queue, summary_file_name):
    print("Starting summary writer", flush=True)

    summary_file_name = summary_file_name + ".pkl"
    def batch_write(buffer_dict):
        if os.path.exists(summary_file_name):
            summary_file = pickle.load(open(summary_file_name, "rb"))
        else:
            summary_file = defaultdict(int)
        
        for key, val in buffer_dict.items():
            summary_file[key] += val
    ...
    elem_in_buffer = 0
    while True:
        next_formula = summary_queue.get()
        if next_formula is None:
            break
            
        buffer[next_formula] += 1
        elem_in_buffer += 1

        if elem_in_buffer >= 1_000_000:
            batch_write(buffer)
            buffer.clear()
            elem_in_buffer = 0
    ...
    if len(buffer) != 0:
        ...
        buffer.clear()
        elem_in_buffer = 0
    print("Summary writer killed", flush=True)
...
def main():
    ...
    summary_queue = Queue(maxsize=1_000_000)
    ...
    summary_file = Process(target=summary_writer, args=(summary_queue, "summary"))
    summary_file.start()

    # start the workers
    workers = []
    for i in range(num_workers):
        p = Process(target=process_file, args=(i, input_queue, writer_queue, summary_queue))
        p.start()
        workers.append(p)
